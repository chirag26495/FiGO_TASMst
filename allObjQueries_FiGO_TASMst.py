...
parser.add_argument('--videopath', type=str, required=True, help='Video path')
parser.add_argument('--startlabel', type=str, required=True, help='object label')
parser.add_argument('--queryoutdir', type=str, required=True, help='Query output directory')
args = parser.parse_args()
# args = parser.parse_args(['--videopath', '../data/birds.mp4', '--startlabel', 'car', '--queryoutdir', 'query_out/'])
print(f"##### INPUTS: \nVideo: {args.videopath}\nObject-label: {args.startlabel}\nQuery_output: {args.queryoutdir}")


NOTEBOOK_RESOURCES_PATH = 'basics_resources'
TASM_STATUS_PATH = NOTEBOOK_RESOURCES_PATH+'/processed_videos_inTASM.json'
QUERY_OUT_DIR = args.queryoutdir #'query_out'
...
allobjectqueries(args.videopath, args.startlabel)
